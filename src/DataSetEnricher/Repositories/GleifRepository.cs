...
public class GleifRepository : IGleifRepository
{
    ...
    public async Task<LeiRecord> GetLeiRecordByLeiNumber(string lei)
    {
        var leiRecordByLeiNumber = await _gleiClient.GetFromJsonAsync<LeiRecord>($"api/v1/lei-records?filter[lei]={lei}");
